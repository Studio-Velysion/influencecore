...
export class ConfigurationChecker {
  ...
  readEnvFromFile() {
    ...
    // In some environments (e.g. Cursor), writing ".env" can be blocked, so we
    // also support "config/dev.env".
    const explicit = process.env.ENV_FILE ? resolve(process.env.ENV_FILE) : null;
    const candidates = [
      ...(explicit ? [explicit] : []),
    ...
    if (!envFile) {
      console.error('Env file not found! Tried:', candidates);
      return;
    }
