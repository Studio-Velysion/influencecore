...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    await requirePermission('MESSA_TEMPLATES_VIEW')
    const { id } = await ctx.params
    const res = await postizFetch(`/templates/${encodeURIComponent(id)}`, { method: 'GET' })
...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const res = await postizFetch(`/templates/${encodeURIComponent(id)}`, {
      method: 'PUT',
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    await requirePermission('MESSA_TEMPLATES_VIEW')
    const { id } = await ctx.params
    const res = await postizFetch(`/templates/${encodeURIComponent(id)}`, { method: 'DELETE' })
    const data = await postizJson(res)
    return NextResponse.json(data, { status: res.status })
  } catch (e: any) {
    return NextResponse.json({ error: e?.message ?? 'Erreur' }, { status: e?.status ?? 500 })
