...
export class AuthMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    // Mode "trusted internal" (ic-billing-core) :
    // - Permet d'utiliser le backend Postiz comme moteur de fonctionnalités sans exposer son UI login/register.
    // - ic-billing-core appelle les endpoints via son serveur (API proxy) en fournissant un header secret.
    const internalKeyHeader = req.headers['x-ic-billing-core-internal-key'];
    const internalKeyEnv = process.env.IC_BILLING_CORE_INTERNAL_KEY;
    if (
      internalKeyEnv &&
      typeof internalKeyHeader === 'string' &&
      internalKeyHeader === internalKeyEnv
    ) {
      const userIdHeader = req.headers['x-ic-billing-core-user-id'];
      const userEmailHeader = req.headers['x-ic-billing-core-user-email'];

      const forcedUserId =
        typeof userIdHeader === 'string'
          ? userIdHeader
          : process.env.IC_BILLING_CORE_POSTIZ_USER_ID;
      const forcedUserEmail =
        typeof userEmailHeader === 'string'
          ? userEmailHeader
          : process.env.IC_BILLING_CORE_POSTIZ_USER_EMAIL;
      ...
      // On ne bloque pas sur activated ici : l'auth est déjà gérée côté ic-billing-core.
      // @ts-ignore
      delete user.password;

      const orgHeader =
        (req.cookies.showorg || req.headers.showorg) ??
        process.env.IC_BILLING_CORE_POSTIZ_ORG_ID;
