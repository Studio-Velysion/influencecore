/**
 * InfluenceCore — Dashboard Data Pack (copier/coller)
 *
 * Ce fichier centralise les "données" des tableaux de bord (routes + cartes + libellés).
 * Objectif: pouvoir copier/coller facilement ce dashboard dans d’autres projets.
 *
 * NOTE:
 * - Les permissions ici sont des STRINGS (ex: "ideas.view").
 * - Dans InfluenceCore, elles correspondent à `lib/permissions.ts`.
 * - Tu peux adapter les permissions / routes selon ton projet.
 */
...
export interface SidebarRouteItem {
  path: string
  label: string
  /** Pour implémentation UI: associe une icône via `iconKey` ou mapping dans ton projet */
  iconKey: IconKey | 'home'
  permission?: PermissionKey
}

/**
 * Libellés utilisés dans la navbar "client"
 */
...
/**
 * Libellés utilisés dans la navbar "admin"
 */
