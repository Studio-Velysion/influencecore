...
/**
 * Options NextAuth partagées entre la Route API NextAuth (App Router)
 * et les appels serveur `getServerSession()`.
 *
 * IMPORTANT: ne pas exporter `authOptions` depuis `app/api/auth/[...nextauth]/route.ts`,
  ...
  callbacks: {
    async jwt({ token, user, account, profile }) {
      ...
      if (account?.provider === 'keycloak') {
        ...
        token.isAdmin = token.roles.includes('influencecore-admin') || token.roles.includes('admin')
      }
