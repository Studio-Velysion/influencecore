...
Vous essayez d'exécuter `npm run dev` depuis le répertoire `ic-billing-core` au lieu de `postiz-app-main`.

**Erreur** :
```
Error: Cannot find module 'H:\Studio Velysion CreatorHub\ic-billing-core\node_modules\next\dist\bin\next'
```
...
**Toujours exécuter les commandes depuis `postiz-app-main`**, pas depuis `ic-billing-core` !
