...
export class AuthController {
  ...
  private isInfluenceCoreInternal(req: Request) {
    const key = req.headers['x-influencecore-internal-key'];
    const env = process.env.INFLUENCECORE_INTERNAL_KEY;
  ...
  async register(
    ...
    // Si InfluenceCore pilote l'auth, on bloque l'auth public Postiz (pas de page register/login Postiz).
    if (process.env.INFLUENCECORE_DISABLE_PUBLIC_AUTH === 'true' && !this.isInfluenceCoreInternal(req)) {
      return response.status(403).send('Auth Postiz désactivée (pilotée par InfluenceCore)');
    }
    try {
