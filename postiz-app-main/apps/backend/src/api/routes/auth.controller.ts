...
export class AuthController {
  ...
  private isic-billing-coreInternal(req: Request) {
    const key = req.headers['x-ic-billing-core-internal-key'];
    const env = process.env.IC_BILLING_CORE_INTERNAL_KEY;
    return Boolean(env && typeof key === 'string' && key === env);
  }
  ...
  async register(
    ...
    // Si ic-billing-core pilote l'auth, on bloque l'auth public Postiz (pas de page register/login Postiz).
    if (process.env.IC_BILLING_CORE_DISABLE_PUBLIC_AUTH === 'true' && !this.isic-billing-coreInternal(req)) {
      return response.status(403).send('Auth Postiz désactivée (pilotée par ic-billing-core)');
    }
    try {
  ...
  /**
   * ic-billing-core SSO bridge:
   * - Called server-to-server by ic-billing-core (must include x-ic-billing-core-internal-key).
   * - Ensures a Postiz user exists (auto-provision), then returns a Postiz JWT + orgId.
   * - ic-billing-core will set browser cookies `auth` + `showorg` and redirect the user to /social.
   */
  @Post('/ic-billing-core/session')
  async ic-billing-coreSession(
    @Req() req: Request,
    ...
    if (!this.isic-billing-coreInternal(req)) {
      return response.status(403).send('Forbidden');
    }

    const emailFromHeader = req.headers['x-ic-billing-core-user-email'];
    const email =
    ...
    if (!user) {
      ...
      const company = body?.company?.trim() || 'ic-billing-core';
      const create = await this._organizationService.createOrgAndUser(
        {
          ...
          providerId: `ic-billing-core:${email}`,
        } as any,
        ip,
        userAgent
      );
